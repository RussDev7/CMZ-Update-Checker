	...
	public class GrenadeLauncherGuidedInventoryItemClass : GrenadeLauncherBaseInventoryItemClass
	{
		...
		public void CheckIfLocked(TimeSpan elapsedGameTime)
		{
			if (this._game.LocalPlayer.Shouldering && EnemyManager.Instance.DragonPosition != Vector3.Zero)
			{
				...
				{
					GrenadeLauncherGuidedInventoryItemClass.tp.Init(position, EnemyManager.Instance.DragonPosition);
					IShootableEnemy e = EnemyManager.Instance.Trace(GrenadeLauncherGuidedInventoryItemClass.tp, false);
					if (!GrenadeLauncherGuidedInventoryItemClass.tp._collides || (e != null && e is DragonClientEntity))
					{
