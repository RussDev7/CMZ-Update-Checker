	...
	public class GrenadeLauncherGuidedInventoryItemClass : GrenadeLauncherBaseInventoryItemClass
	{
		...
		public void CheckIfLocked(TimeSpan elapsedGameTime)
		{
			if (this._game.LocalPlayer.Shouldering && EnemyManager.Instance.DragonPosition != Vector3.Zero)
			{
				...
				{
					GrenadeLauncherGuidedInventoryItemClass.tp.Init(worldPosition, EnemyManager.Instance.DragonPosition);
					IShootableEnemy shootableEnemy = EnemyManager.Instance.Trace(GrenadeLauncherGuidedInventoryItemClass.tp, false);
					if (!GrenadeLauncherGuidedInventoryItemClass.tp._collides || (shootableEnemy != null && shootableEnemy is DragonClientEntity))
					{
