...
namespace DNA.CastleMinerZ.Terrain.WorldBuilders
{
	internal class CostalBiome : Biome
	{
		...
		public override void BuildColumn(BlockTerrain terrain, int worldX, int worldZ, int minY, float terrainblender)
		{
			...
			num2 += this._noiseFunction.ComputeNoise(0.00234375f * (float)worldX, 0.00234375f * (float)worldZ);
			num2 += this._noiseFunction.ComputeNoise(0.0046875f * (float)worldX, 0.0046875f * (float)worldZ) / 2f;
			bool flag = false;
			...
			num2 = this._noiseFunction.ComputeNoise((float)worldX * 0.0625f, (float)worldZ * 0.0625f) * 10f;
			int num12 = (int)num2;
			if (num12 > 1)
			{
			}
					...
					else
					{
						...
						else
						{
							if (j < 32)
							{
								if (j < 16)
								{
									num2 = this._noiseFunction.ComputeNoise(0.375f * (float)(worldX + 1750), 0.375f * (float)(num13 + 1750), 0.375f * (float)(worldZ + 1750));
									if (num2 > 0.75f)
									{
										terrain._blocks[num14] = Biome.diamondsBlock;
									}
								}
								num2 = this._noiseFunction.ComputeNoise(0.25f * (float)(worldX + 777), 0.25f * (float)(num13 + 777), 0.25f * (float)(worldZ + 777));
								if (num2 < -0.75f)
								{
									terrain._blocks[num14] = Biome.goldBlock;
								}
								num2 = this._noiseFunction.ComputeNoise(0.25f * (float)(worldX + 5432), 0.25f * (float)(num13 + 5432), 0.25f * (float)(worldZ + 5432));
								if (num2 > 0.8f)
								{
									terrain._blocks[num14] = Biome.surfaceLavablock;
								}
							}
							num2 = this._noiseFunction.ComputeNoise(0.25f * (float)(worldX + 250), 0.25f * (float)(num13 + 250), 0.25f * (float)(worldZ + 250));
							if (num2 > 0.74f)
							{
								terrain._blocks[num14] = Biome.ironBlock;
							}
							num2 = this._noiseFunction.ComputeNoise(0.3125f * (float)(worldX + 1000), 0.3125f * (float)(num13 + 1000), 0.3125f * (float)(worldZ + 1000));
							if (num2 < -0.5f)
							{
